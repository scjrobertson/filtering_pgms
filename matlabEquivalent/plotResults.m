function plotResults(model, groundTruth, measurements, cardinalityEstimate)
...
for i = 1:simulationLength
    [eOspa(1, i), eOspa(2, i), eOspa(3, i)] = euclideanOspa(groundTruth.means{i}, groundTruth.means{i}, model.eOspaC, model.ospaP);
end
% Hellinger OSPA
hOspa = zeros(3, simulationLength);
for i = 1:simulationLength
    [hOspa(1, i), hOspa(2, i), hOspa(3, i)] = hellingerOspa(groundTruth.means{i}, groundTruth.covariances{i}, ...
        groundTruth.means{i}, groundTruth.covariances{i}, model.hOspaC, model.ospaP);
end
...
for i = 1:simulationLength
    projectedStateEstimate{i} = model.C*groundTruth.means{i};
end
%% Plot the observation space
...
% State Estimates
for i = 1:simulationLength
    estimateLine = line(i-1, projectedStateEstimate{i}(1, :), 'LineStyle','none','Marker','*','Markersize',12,'Color', 'red');
end
% Limits
...
% State Estimates
for i = 1:simulationLength
     estimateLine = line(i-1, projectedStateEstimate{i}(2, :), 'LineStyle','none','Marker','*','Markersize',12,'Color', 'red');
end
% Limits
xlabel('t (s)'); ylabel('y (m)');
set(gca, 'XLim', [0 simulationLength-1]); set(gca, 'YLim', xLimits);
...
set(gca, 'XLim', [0 simulationLength-1]); set(gca, 'YLim', [0 max(eOspa(1, :))+0.1]);
...
set(gca, 'XLim', [0 simulationLength-1]); set(gca, 'YLim', [0 max(hOspa(1, :))+0.1]);
...
maximumCardinality = max(max(groundTruth.cardinality), max(cardinalityEstimate));
